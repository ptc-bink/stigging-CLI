...
import { userList } from "@/app/utils/_data";
        ...
        <div className="flex fixed top-0 left-0 w-screen h-screen bg-[#141518]/30 backdrop-blur-sm justify-center items-center">
            <div className="flex flex-col w-[450px] rounded-md p-6 gap-6 border border-cgrey-200 bg-cgrey-100">
                <div className="flex justify-between gap-4">
                    <p className="text-base text-white font-semibold">Permitted Users</p>
                    <Image
                        src={Cancel}
                        width="24"
                        height="24"
                ...
                <div className="flex flex-col gap-2">
                    ...
                    <button className="flex justify-center items-center rounded-md outline-none bg-white border border-[#EEEEEE] text-[#16171B] text-sm leading-4 font-medium p-3" onClick={handleAddUser}>
                        Add user
                    </button>
                </div>
                <div className="flex flex-col px-1 py-[10px] gap-[6px] rounded-lg bg-[#141518] border overflow-y-auto  border-[#292A2E] max-h-[235px]">
                    {userList.map((item, index) => (
                        <div className={`text-sm cursor-pointer leading-[18px] font-medium hover:text-white hover:bg-cgrey-100 text-[#939393] ${flags[index] ? "bg-cgrey-100" : ""}`} onClick={() => handleSetUser(item, index)}>{item}</div>
                    ))}
                </div>
            </div>
        </div >
